...
function Dashboard() {
  ...
  const summary = useMemo(() => {
    ...
    const avgMoisture = totalFarms
      ? Math.round(
          farms.reduce((sum, farm) => sum + clampPct(farm.soilMoisture || 0), 0) / totalFarms
        )
      : 0;
    const totalWater = farms.reduce((sum, farm) => sum + (farm.waterUsed || 0), 0);
    const counts = farms.reduce(
      ...
      { Critical: 0, Dry: 0, Optimal: 0, Wet: 0 }
    );
    return { totalFarms, avgMoisture, totalWater, counts };
  }, [farms]);
    ...
    <DashboardLayout farms={farms} weather={weatherData}>
        ...
        <div>
          ...
          {activeFarm && (
            <span className="inline-flex items-center gap-2 mt-3 text-xs font-semibold px-3 py-1 rounded-full bg-green-50 text-green-800 border border-green-100">
              Active farm: {activeFarm.cropType || activeFarm.location || "Unnamed"}
            </span>
          )}
        </div>
      ...
      <div className="grid md:grid-cols-4 gap-4 mt-6">
        <div className="bg-white p-4 rounded-2xl shadow">
          ...
          <p className="text-2xl font-semibold text-green-900">{summary.totalFarms}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow">
          <p className="text-sm text-gray-500">Average moisture</p>
          <p className="text-2xl font-semibold text-green-900">{summary.avgMoisture}%</p>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow">
          <p className="text-sm text-gray-500">Total water used</p>
          <p className="text-2xl font-semibold text-green-900">{summary.totalWater} L</p>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow">
